...
class VoxForge(Corpus):
    ...
    DATASET_URLS = {
        "train": ["https://www.dropbox.com/s/wrguetal6xmrgta/voxforge-ptbr.tar.gz?dl=1"]
    }
    ...
    def get_data(self, root_dir, set_type):
        ...
        search_dirs = set(
            [os.path.split(p)[0].replace('{}wav'.format(os.path.sep), '') for p in audio_paths])
        ...
        for curr_dir in search_dirs:
            ...
            assert os.path.isfile(
                (transcriptions_file)), "prompts.txt not found in {}".format(root_dir)
            ...
            with open(transcriptions_file, 'r', encoding='utf8') as f:
                for line in f.readlines():
                    ...
                    pattern = re.compile(r'{}(?:[\\\/a-z]*){}(?:.[a-z]+)$'.format(name, curr_id))

                    filtered_audio_paths = list(filter(pattern.findall, audio_paths))

                    if len(filtered_audio_paths) != 1:
                        print((
                            'WARNING: Found zero or more than one audio file for the transcription '
                            'id {} in {}. Skipping...').format(curr_id, transcriptions_file))
                        continue
