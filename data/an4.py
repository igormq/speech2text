...
class AN4(Corpus):
    ...
    DATASET_URLS = {
        "train": ["http://www.speech.cs.cmu.edu/databases/an4/an4_raw.bigendian.tar.gz"],
        "test": ["http://www.speech.cs.cmu.edu/databases/an4/an4_raw.bigendian.tar.gz"]
    }
    ...
    def get_data(self, root_dir, set_type):
        ...
        file_ids_path = os.path.join(root_dir, 'etc/an4_{}.fileids'.format(set_type))
        transcripts_path = os.path.join(root_dir, 'etc/an4_{}.transcription'.format(set_type))

        data = []
        with open(file_ids_path, 'r') as f, open(transcripts_path, 'r') as t:

            for audio_path, transcript in zip(f, t):
                audio_path = os.path.join(wav_path, '{}.raw'.format(audio_path.strip()))
    ...
    def process_transcript(self, root_dir, transcript_path, audio_path):
        return transcript_path.split('(')[0].strip("<s>").split('<')[0].strip().upper()
