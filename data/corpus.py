...
DEFAULT_AUDIO_FILE_EXT = ['aac', 'au', 'flac', 'm4a', 'mp3', 'ogg', 'wav']
...
class Corpus(object):
            ...
            for url in urls:
                ...
                if url is not None:
                    ...
                    assert os.path.exists(
                        extracted_dir), "Archive {} was not properly uncompressed.".format(fname)

                else:
                    print('No URL found. Skipping download.')
                    curr_extracted_dir = extracted_dir

                    assert os.path.exists(extracted_dir), 'No folder found in {}'.format(
                        extracted_dir)
    ...
    def _create_manifest(self, data_path, name, ordered=True, prune=False):
        ...
        for wav_path in tqdm(file_paths, total=len(file_paths), desc='Get durations'):
            transcript_path = wav_path.replace('{0}wav{0}'.format(os.path.sep),
                                               '{0}txt{0}'.format(os.path.sep)).replace(
                                                   '.wav', '.txt')
        ...
        df = pd.DataFrame(
            OrderedDict(
                audio_paths=wav_files, transcriptions=transcripts_files, durations=durations))
    ...
    def _wav_txt_split(self, files_dir, wav_dir, txt_dir, set_type):
        ...
        if len(file_paths) == 0:
            raise IOError('No audio file found at {}'.format(files_dir))
    ...
    def find_audios(self, root_dir):
        return set(
            ...
            if os.path.splitext(f)[1].lower().replace('.', '') in DEFAULT_AUDIO_FILE_EXT)
    ...
    def process_data(self, root_dir, audio_path, transcript_path, wav_dir, txt_dir):
        ...
        unique_name = os.path.splitext(os.path.relpath(audio_path, start=root_dir))[0].replace(
            os.sep, '-')
